package common.flytext
{
	...
	public class FlyTextHelper
	{
		...
		private static function showDamage(item:TargetObject, isNormalSpell:Boolean, isMainCharPet:Boolean = false):void
		{
			...
			var target:SceneCharacter = target = GameInstance.scene.getCharacterById(item.guid);
			if (target == null)
			{
				return;
			}
			for each (var flag:int in mutiTypeArr)
			{
				...
				if (flytext)
				{
					...
					{
						target.showFlyText(setBattleText(flag, flytext, target.isMainChar, isNormalSpell, isMainCharPet));
					}
				}
			}
			if (item.miss_damage > 0)
			{
				...
				target.showFlyText(setBattleText(ValueEffects.Miss_Damage, missFlytext, target.isMainChar, isNormalSpell));
			}
			if (target)
			{
		 ...
		 *
		 */
		public static function addStatusFlyText(character:SceneCharacter, type:int, value:int):void
		{
			var isMainChar:Boolean = character.isMainChar;
		 ...
		 *
		 */
		public static function addPropertyFlyText(char:SceneCharacter, type:int, value:int = 0):void
		{
		 ...
		 *
		 */
		public static function addSpellNameFlyText(caster:SceneCharacter, name:String):void
		{
			if (name.length == 0)
			{
				return;
