package fj1.common.ui
{
	...
	public class TWindowManager
	{
		...
		private var _window_mediator:Dictionary = new Dictionary();
...
//			PopupManager.instance.removeAllPopup();
		 ...
		 *
		 */
		public function registerWindowMediator(panel:Class, mediator:Class, mapMediator:Function):void
		{
			if (panel.hasOwnProperty("NAME"))
			{
				var name:String = panel.NAME;
				if (!_window_mediator.hasOwnProperty(name))
					_window_mediator[name] = {panel: panel, mediator: mediator, onComplete: mapMediator};
			}
			else
				log.warn("窗口 " + panel + "必须指定NAME(需与windowskin，name配置相同)");
		}
		...
		public function showPopup2(parentWindow:TWindow, name:String, modal:Boolean = false, useCover:Boolean = false, managerModel:int = MODEL_USE_OLD, offset:Point = null, parms:Object = null, dataParms:Object =
			null, onComplete:Function = null):DisplayObject
		{
...
//			}
			if (needCreate)
			{
				...
				if (!newV)
				{
					var object:Object = _window_mediator[name];
					if (object) //已注册
					{
						WindowSkinMannager.instance.watingShow(name, [object, parentWindow, modal, useCover, parms, dataParms], onComplete);
						return null;
					}
					else
					{
						log.warn("窗口 " + name + "的Mediator未注册");
					}
					return null;
				}
				this.showPopup(parentWindow, newV, modal, useCover, offset, parms, dataParms);
