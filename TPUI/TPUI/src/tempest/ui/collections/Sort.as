package tempest.ui.collections
{
	...
	public class Sort extends EventDispatcher
	{
								 ...
								 compareFunction:Function = null):int
		{
			...
			if (!items)
			{
				throw new Error("collections Error: noItems");
			}
			...
			if (compareFunction == null)
			{
				...
				if (values && fieldList.length > 0)
				{
					...
					for (var i:int = 0; i < fieldList.length; i++)
					{
						...
						if (fieldName)
						{
							...
							if (hasFieldName)
							{
								...
								{
									throw new Error("collections Error: findCondition " + fieldName);
								}
								else
								{
					...
					{
						throw new Error("collections Error: findRestriction");
					}
					else
					{
						try
				...
				switch(direction)
				{
					...
					case 0:
						...
						switch(mode)
						{
							...
							default:
							{
								throw new Error("collections Error: unknownMode");
							}
						} // switch
						break;
		...
		public function sort(items:Array):void
		{
			...
			if (usingCustomCompareFunction)
			{
				...
				if (unique)
				{
					...
					if (uniqueRet1 == 0)
					{
						throw new Error("collections Error: nonUnique");
					}
				}
				else
				{
		...
		private function noFieldsCompare(a:Object, b:Object, fields:Array = null):int
		{
			if (!defaultEmptyField)
			{
				...
				catch(e:Error)
				{
					...
					throw new Error("collections Error: noComparator");
				}
			}
