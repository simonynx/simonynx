package tempest.utils
{
	...
	public final class Geom
	{
		...
		{
			//八方向 360/8=45，向右为0，逆时针角度为负，顺时针为正  
			//0-东  90-南 180-西
			return (Math.round((Math.atan2(p2.y - p1.y, p2.x - p1.x) * RAD_2_RANGLE + 90) / 45) + 8) % 8; //角色面向正北为0  角度+90
		}
		...
		public static function GetEndPoint(beginPoint:Point, mousePoint:Point, distance:Number):Point
		{
			...
			{
				var radian:Number=Geom.getTwoPointRadian(beginPoint, mousePoint) + Math.PI;
				p=new Point(Math.cos(radian) * distance + beginPoint.x, Math.sin(radian) * distance + beginPoint.y);
			}
			return p;
		}
