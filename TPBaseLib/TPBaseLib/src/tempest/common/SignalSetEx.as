package tempest.common
{
	...
	public class SignalSetEx
	{
		protected const _signals:Dictionary = new Dictionary();

		public function SignalSetEx()
		{
			super();
		}

		public function setSignal(name:String, signal:ISignal):void
		{
			if (!name)
		...
		public function removeSignal(name:String):void
		{
			if (!name)
				throw new ArgumentError('name must not be null.');
		...
		public function getSignal(name:String, signalClass:Class = null, ... parameters):ISignal
		{
			if (null == name)
				throw new ArgumentError('name must not be null.');
			var ret:ISignal = _signals[name];
			if (!ret)
			{
				if (!signalClass)
				{
				...
				ret = ClassUtil.getInstanceByClass(signalClass, parameters);
			}
			return ret;
		}
