package tempest.common.rsl
{
	...
	{
		private static const log:ILogger = TLog.getLogger(TRslLoader);
		...
		private function ErrorHandler(event:ErrorEvent):void
		{
			...
			_clearUrlLoader();
			log.error("TRslLoader UrlLoadError url:{0}", _url);
		...
		private function completeHandler(event:Event):void
		{
			...
			if (data == null)
			{
				log.error("Urlloader data Error.url:{0}", _url);
		...
		private function completeHandler2(event:Event):void
		{
			...
			if (!_content)
			{
				log.error("data is empty.url:{0}" + _url);
				dispose();
