package br.com.stimuli.loading.lazyloaders
{
	...
	/**
	 * JSON文件加载器
	 *       @example Basic usage:<listing v="3.0">
	...
	{
		private static const log:ILogger = TLog.getLogger(LazyJSONLoader);
		private var _decodeFunc:Function;

		function LazyJSONLoader(url:*, name:String, numConnections:int = BulkLoader.DEFAULT_NUM_CONNECTIONS, logLevel:int = TLog.LEVEL_ERROR)
		{
		...
		public function get decodeFunc():Function
		{
			if (!Boolean(_decodeFunc))
			{
				...
				var decoderClass:Object = getDefinitionByName("LwSysEngine.IO.serialization.json.JSON");
		...
		lazy_loader override function _lazyParseLoader(withData:String):void
		{
			...
			log.logLevel = source["logLevel"] || TLog.LEVEL_ERROR;
			if (source["name"])
			{
				_name = source["name"];
			}
			...
			for each (var fileProp:Object in source["files"])
			{
				...
				if (!String(props["url"]))
				{
					log.warn("got a item files with no url, ignoring");
					continue;
				}
				if (props["context"])
				{
