...
package com.riaidea.text
{
	...
	public class RichTextField extends Sprite
	{
		...
		public function insertSprite(newSprite:Object, index:int = -1, autoRender:Boolean = true, cache:Boolean = false):void
		{
			...
			if (spriteObj == null)
				throw Error("Specific sprite:" + newSprite + " is not a valid display object!");
...
//			if(stage)
//			{
//				stage.quality = stage.quality;//强制刷新位图缓存
//			}
			_textRenderer.width = width;
			_textRenderer.height = height;
			_width = _textRenderer.width;
			_height = _textRenderer.height;
//			this.scrollRect = new Rectangle(0, 0, _textRenderer.width, _textRenderer.height);
			_spriteRenderer.render();
		...
		public function importXML(data:XML):void
		{
			...
			for (var i:int = 0; i < data.sprites.sprite.length(); i++)
			{
				...
				if (html && contentLength > 0 && index > 0)
		...
		private var spriteTempBuff:Array = [];
		...
		public function append2(newText:String, newSprites:Array = null, autoWordWrap:Boolean = false, format:TextFormat = null):void
		{
			...
			if (html)
			{
				//过滤html标签
				reg = /<.*?>/g;
				newText = newText.replace(reg, function():String
				{
					if (arguments[0] == "<br>")
						return "\n";
					else
			...
			while (true)
			{
				var match:Object = reg.exec(newText);
				if (!match)
				{
					break;
				}
				spriteTempBuff.push({src: newSprites[spriteTempBuff.length], index: match.index}); //记录插入位置
				var _newText:String = newText.replace(reg, "");
				//				var _newText:String = newText.replace(reg, function():String
				//				{
				//					spriteTempBuff.push({src:newSprites[spriteTempBuff.length], index:arguments[1]});//记录插入位置
				//					return "";
				//				});
				if (_newText == newText)
					break;
			...
			reg = /{.*?}/g;
			this.append(oldText.replace(reg, ""), spriteTempBuff, autoWordWrap, format);
		}
	}
}
