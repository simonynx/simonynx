...
package com.riaidea.text
{
	...
	internal class SpriteRenderer
	{
		...
		private var _firstAdded:Boolean;
		...
		public function SpriteRenderer(rtf:RichTextField)
		{
			...
			_rtf.addEventListener(Event.ADDED_TO_STAGE, onAddedToStage);
			_rtf.addEventListener(Event.REMOVED_FROM_STAGE, onRemovedFromStage);
		}
		...
		{
			//版本 10.3.183.5 中，文本带滤镜时，才有可能出现竖排问题
			if (Capabilities.version.split(" ")[1] == "10,3,183,5" && _rtf.textfield.filters != null)
			{
				if (_rtf.text == "" || _rtf.textfield.numLines >= _rtf.text.replace(/[\s]/g, "").length)
				{
					_delayCount = 0;
					_rtf.textfield.addEventListener(Event.ENTER_FRAME, onDelayRender);
				}
				else
				{
					_doRender();
			...
			{
				//添加到Stage后第一次渲染，强制刷新位图缓存，保证getCharBoundaries函数结果正确
//				if(_firstAdded)
//				{
//					_firstAdded = false;
////					_rtf.stage.quality = _rtf.stage.quality;//强制刷新位图缓存
//					_delayCount = 0;
//					_rtf.textfield.addEventListener(Event.ENTER_FRAME, onDelayRender);
//				}
//				else
...
//				}
		...
		private function onDelayRender(event:Event):void
		{
			_delayCount++;
			if (_delayCount == _dalayMax)
			{
				_delayCount = 0;
		...
		internal function removeSprite(index:int):void
		{
			...
			if (sprite != null)
			{
				if (_spriteContainer.contains(sprite))
					_spriteContainer.removeChild(sprite);
		...
		internal function clear():void
		{
			while (_spriteContainer.numChildren > 0)
				_spriteContainer.removeChildAt(0);
			for (var p:* in _spriteIndices)
				delete _spriteIndices[p];
			_numSprites = 0;
		}
